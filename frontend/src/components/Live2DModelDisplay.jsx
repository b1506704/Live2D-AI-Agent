...
const Live2DModelDisplay = forwardRef(({ modelPath }, ref) => {
  const canvasRef = useRef();
  const appRef = useRef();
  const modelRef = useRef();
  const mouthInterval = useRef();
  ...
  useEffect(() => {
    ...
    const app = new PIXI.Application({
      ...
      // Responsive: use parent size, fallback to 400x600
    ...
    function fitModelToCanvas(model) {
      ...
      // Zoom: fill width, and zoom in for upper body (show ~45% of height, larger scale)
      const scale = Math.min(cWidth / mWidth, (cHeight / 0.45) / mHeight) * 1.25;
      model.scale.set(scale, scale);
      model.x = cWidth / 2;
      // Center upper half: move model up so upper body is visible
      model.y = cHeight * 0.38;
      model.anchor.set(0.5, 0.15); // anchor even higher for more upper body
    }
    async function applyPoseAndIdle(model) {
      // Try to load pose3.json if available
      ...
      // Only play idle motion if not currently playing a custom motion
    ...
    if (isCubism4) {
      import('pixi-live2d-display/cubism4').then(({ Live2DModel }) => {
        Live2DModel.from(modelPath).then(async model => {
  ...
  // Prevent pointer events until model is loaded
  useEffect(() => {
    const canvas = canvasRef.current;
    function handlePointerMove(e) {
      if (!modelLoaded.current || !modelRef.current || !canvasRef.current) return;
      if (isMotionPlaying) return; // Disable mouse-follow during motion
      const rect = canvasRef.current.getBoundingClientRect();
      // Use window mouse position relative to canvas center
      ...
      // Set model parameters for head and eyes
  ...
  // Expose playTTSAndAnimate to parent via ref
  useImperativeHandle(ref, () => ({
    async playTTSAndAnimate(text, pitch, speed, voice, gender, language, mood, onAudioStart, onAudioEnd) {
      ...
      try {
        ...
        audio.onplay = () => { if (onAudioStart) onAudioStart(); startAudioLipSync(audio); triggerExpressiveMotion(text); };
        audio.onended = () => { if (onAudioEnd) onAudioEnd(); stopLipSync(); URL.revokeObjectURL(url); };
        audio.onerror = (e) => { console.error('Audio playback error', e); alert('Audio playback error: The TTS audio could not be played.'); };
        audio.play();
      } catch (e) {
        ...
        if ('speechSynthesis' in window) {
          ...
          utter.pitch = pitch;
          utter.rate = speed;
          utter.onstart = () => { if (onAudioStart) onAudioStart(); startLipSync(); triggerExpressiveMotion(text); };
          utter.onend = () => { if (onAudioEnd) onAudioEnd(); stopLipSync(); };
          window.speechSynthesis.speak(utter);
        } else {
          if (onAudioStart) onAudioStart();
          startLipSync(); triggerExpressiveMotion(text); setTimeout(() => { stopLipSync(); if (onAudioEnd) onAudioEnd(); }, 2000);
        }
      }
    },
    setExpression(expressionName) {
      if (!modelRef.current) return;
      // Try Cubism 4 first
      if (modelRef.current.internalModel && modelRef.current.internalModel.motionManager && modelRef.current.internalModel.motionManager._expressions) {
    ...
    playMotion(motionName, priority = 'NORMAL') {
      if (!modelRef.current) return;
      setIsMotionPlaying(true);
      console.log('[Live2D] playMotion called:', motionName, priority);
      // Use the modern API if available
      if (typeof modelRef.current.motion === 'function') {
        modelRef.current.motion(motionName, priority);
        // Assume typical motion duration ~2s, reset after
        setTimeout(() => setIsMotionPlaying(false), 2000);
        return;
      }
      // Fallback to legacy Cubism 4 logic if needed
      if (modelRef.current.internalModel && modelRef.current.internalModel.motionManager && modelRef.current.internalModel.motionManager._motions) {
        ...
        for (const group in motions) {
          for (let i = 0; i < motions[group].length; i++) {
            ...
            if ((m.name && m.name.toLowerCase().includes(motionName.toLowerCase())) ||
  ...
  return (
    <div className="w-full h-full min-w-[350px] min-h-[500px] flex items-center justify-center max-w-full max-h-full p-0 md:p-0" style={{ position: 'relative', width: '50vw', height: '80vh' }}>
      <canvas
        ref={canvasRef}
